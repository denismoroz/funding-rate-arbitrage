...
def _tick(coin: str = "BTC", offset_s: int = 0, rate: float = 0.0001) -> FundingTick:
...
def test_add_one_tick() -> None:
    state = CoinState("BTC", 12)
    t = _tick(coin="BTC", offset_s=0, rate=0.0001)
    state.add_funding(t)
...
def test_fill_window_makes_ready() -> None:
    ...
    for i, rate in enumerate(rates):
        state.add_funding(_tick(coin="BTC", offset_s=i * 1000, rate=rate))
...
def test_market_add_funding_routes_to_coin() -> None:
    ms = MarketState(["BTC", "ETH"], 12)
    ms.add_funding(_tick(coin="BTC", offset_s=0, rate=0.0001))
...
def test_market_add_funding_unknown_coin_raises() -> None:
    ms = MarketState(["BTC", "ETH"], 12)
    with pytest.raises(KeyError, match="unknown coin"):
        ms.add_funding(_tick(coin="SOL", offset_s=0, rate=0.0001))


def test_market_add_funding_batch() -> None:
    ms = MarketState(["BTC", "ETH"], 12)
    ticks = [
        _tick(coin="BTC", offset_s=1000, rate=0.0001),
        _tick(coin="ETH", offset_s=1001, rate=0.0001),
        _tick(coin="BTC", offset_s=2000, rate=0.0001),
        _tick(coin="ETH", offset_s=2001, rate=0.0001),
        _tick(coin="BTC", offset_s=3000, rate=0.0001),
    ]
...
def test_market_signals_per_coin() -> None:
    ...
    ms.add_funding(_tick(coin="BTC", offset_s=1000, rate=0.0001))
    ms.add_funding(_tick(coin="BTC", offset_s=2000, rate=0.0002))
    ms.add_funding(_tick(coin="ETH", offset_s=1001, rate=0.0003))
    signals = ms.signals()
