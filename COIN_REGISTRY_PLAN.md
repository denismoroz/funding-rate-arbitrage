...
- **Прод НЕ мигрировать/деплоить в этих фазах** — только read-only копия прод-БД для теста
  миграции. Деплой — отдельный шаг под контролем пользователя.
...
### Фаза 0 — свежая прод-БД локально (read-only)
...
- НЕ мигрировать/деплоить прод в этих фазах (только read-only копия БД; деплой — отдельно).
