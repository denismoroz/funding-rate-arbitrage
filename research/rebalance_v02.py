"""
rebalance_v02.py — Scale-in/scale-out + rotation by funding strength, v0.2.

Three fixes vs v0.1:
  A. Frozen `growing` slots have an exit path: if filter stops passing mid-ramp,
     compute partial-position breakeven and unwind if reached.
  B. Asymmetric entry/exit defaults: entry=15%, entry_24h=12%, exit=12%
     (3-bps gap prevents thrashing on edges).
  C. Window-aware pct_time_in_usdc: per-hour boolean is_idle[h] tracked,
     metric computed as mean over window slice (not global constant).

Run: uv run python research/rebalance_v02.py
"""
...
def _run_simulation(
    ...
    *,
    ...
    n_main_cap: int,
    aave_apr: float,
    unwind_frozen: bool,   # Fix A: if False, skip breakeven-unwind check (v0.1 behavior)
) -> tuple:
    """
    ...
    Aave budget: fixed budget = max(2*TOTAL_CAPITAL, peak_active_capital_observed).
    is_idle_arr: per-hour boolean array — True when all slots are 'empty'.
                 Used for window-aware pct_time_in_usdc (Fix C).
    """
    ...
    is_idle_arr             = np.zeros(n_hours, dtype=bool)  # Fix C

    # ── Counters ───────────────────────────────────────────────────────────────
    n_rotations      = 0
    n_ramps          = 0
    n_frozen_unwinds = 0   # Fix A counter
                ...
                if (not np.isnan(c_ma) and not np.isnan(c_min)
                    ...
                    if unwind_frozen and sl["tranches"]:
                        ...
                        if unreal >= 0:
                            sl["state"] = "shrinking"
                            n_frozen_unwinds += 1
                    # else: stay growing, wait for next tick (filter may resume or breakeven reached)
...
def simulate_rebalance_v02(
    ...
    *,
    ...
    entry_ma_apr: float = 0.15,        # Fix B: tighter default
    entry_min_24h_apr: float = 0.12,   # Fix B: tighter default
    rotation_delta_apr: float = 0.10,
    exit_signal_threshold_apr: float = 0.12,  # Fix B: tighter default
    signal_window_hours: int = 12,
    n_slots_total: int = 4,
    n_main_cap: int = 2,
    aave_apr: float = 0.05,
    unwind_frozen: bool = True,        # Fix A toggle
) -> tuple:
    """
    Returns (pnl_per_hour, active_capital_per_hour, gross_funding_per_hour,
             fees_per_hour, aave_income_per_hour, is_idle_arr, info_dict).
...
def main():
    ...
    configs = [
        ("aave_only",       None,                                                      True),
        ("v02_default",     dict(tick_hours=24, slice_pct=0.10, entry_ma_apr=0.15,
                                 entry_min_24h_apr=0.12, rotation_delta_apr=0.10,
                                 exit_signal_threshold_apr=0.12),                      True),
        ("v02_no_unwind",   dict(tick_hours=24, slice_pct=0.10, entry_ma_apr=0.15,
                                 entry_min_24h_apr=0.12, rotation_delta_apr=0.10,
                                 exit_signal_threshold_apr=0.12),                      False),
        ("v02_loose",       dict(tick_hours=24, slice_pct=0.10, entry_ma_apr=0.10,
                                 entry_min_24h_apr=0.08, rotation_delta_apr=0.05,
                                 exit_signal_threshold_apr=0.08),                      True),
        ("v02_fast_tick",   dict(tick_hours=6,  slice_pct=0.10, entry_ma_apr=0.15,
                                 entry_min_24h_apr=0.12, rotation_delta_apr=0.10,
                                 exit_signal_threshold_apr=0.12),                      True),
    ]
    ...
    for cfg_name, overrides, unwind_frozen in configs:
        ...
        (pnl, cap, gross_fund_arr, fees_arr, aave_arr, is_idle_arr, info) = simulate_rebalance_v02(
        ...
        for period_name, start_idx in [("full", 0), ("last_90d", max(0, n_hours - LAST_90D_HOURS))]:
            ...
            idle_slice   = is_idle_arr[start_idx:]   # Fix C
    ...
    out_path = Path(__file__).parent / "rebalance_v02_results.csv"
    ...
    print("\n" + "=" * 160)
    print("REBALANCE V0.2 — FULL PERIOD (sorted by calmar)")
    print("=" * 160)
    print(df_full[display_cols].sort_values("calmar", ascending=False).to_string(index=False))

    print("\n" + "=" * 160)
    print("REBALANCE V0.2 — LAST 90 DAYS (sorted by calmar)")
    print("=" * 160)
    print(df_90[display_cols].sort_values("calmar", ascending=False).to_string(index=False))

    print("\n" + "=" * 160)
    print("FIX A vs NO-UNWIND comparison (default vs no_unwind):")
    print("=" * 160)
    for period in ("full", "last_90d"):
        r_def = df_out[(df_out["config"] == "v02_default") & (df_out["period"] == period)]
        r_nou = df_out[(df_out["config"] == "v02_no_unwind") & (df_out["period"] == period)]
        if not r_def.empty and not r_nou.empty:
            a_def = r_def["annual_pct"].values[0]
            a_nou = r_nou["annual_pct"].values[0]
            print(f"  [{period:8s}] v02_default={a_def:.2f}%  v02_no_unwind={a_nou:.2f}%  "
                  f"delta={a_def - a_nou:+.2f}%")

    print("\n" + "=" * 160)
    print("FIX C: pct_time_in_usdc full vs last_90d (should differ):")
    print("=" * 160)
    for cfg in ["v02_default", "v02_no_unwind", "v02_loose", "v02_fast_tick"]:
        r_full = df_out[(df_out["config"] == cfg) & (df_out["period"] == "full")]
        r_90   = df_out[(df_out["config"] == cfg) & (df_out["period"] == "last_90d")]
        if not r_full.empty and not r_90.empty:
            p_full = r_full["pct_time_in_usdc"].values[0]
            p_90   = r_90["pct_time_in_usdc"].values[0]
            print(f"  {cfg:20s}  full={p_full:.4f}  last_90d={p_90:.4f}  diff={p_90 - p_full:+.4f}")
