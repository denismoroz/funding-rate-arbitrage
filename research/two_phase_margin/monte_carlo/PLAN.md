...
## T8 — Walk-forward (сиблинг-трек; отвечает на «переобучены ли ПАРАМЕТРЫ»)

MC ответил «робастен ли ЭДЖ к альтернативным путям». Walk-forward отвечает на
другой вопрос: **не переподогнаны ли ПАРАМЕТРЫ** (entry_threshold, phase2_exit,
neg_stop, min_hold) под историю. Метод: тюним на in-sample окне, проверяем на
невидимом out-of-sample; деградация IS→OOS = мера оверфита.
...
**Данные.** Брать **BTC/ETH/SOL** (история 2023-06 → 2026, охватывает hot+cold).
Полный 5-монетный сет НЕ годится — HYPE/PURR цена только с 2025-11 (cold-only),
нет train/test через режимы. Грузить через движковый `load_coin_df` (по пути файла).

**Движок.** Переиспользовать `two_phase_margin.simulate(..., sizing="prod_slot")`
через `engine_adapter` — НЕ переписывать. Метрики — `monte_carlo.metrics`.

**Do.** `monte_carlo/walk_forward.py`:
- Rolling-окна: train = 12 мес, test = 3 мес, шаг = 3 мес (параметризуемо).
- На каждом TRAIN-окне: перебрать НЕБОЛЬШУЮ сетку параметров (например
  `entry_threshold_apr ∈ {0.05,0.10,0.15,0.20}`, `phase2_exit_threshold ∈
  {-0.05,-0.10}` — сетку держать маленькой ради скорости), выбрать лучшую по
  in-sample метрике (annual или Calmar на prod_slot, на занятый капитал).
- Применить выбранные параметры к СЛЕДУЮЩЕМУ test-окну → записать OOS-метрику.
- **Бейзлайн:** прод-дефолтные параметры, применённые ко всем test-окнам без тюнинга.
- Агрегировать: средняя IS vs OOS доходность/Calmar; tuned-OOS vs static-baseline-OOS.
- Записать `WALK_FORWARD_REPORT.md`: таблица по фолдам (train-период, выбранные
  параметры, IS-метрика, OOS-метрика) + сводка деградации + сравнение tuned vs static.

**Контекст для интерпретации** (memory `project_quant_research`): на широком
исследовании single-param тюнинг оказался ловушкой (45%→22% live), а param-ENSEMBLE
держался. Так что ожидаемый вывод — тюнинг скорее переобучается; проверить это.

**Acceptance** (`tests/test_walk_forward.py`):
- walk_forward отрабатывает на BTC/ETH/SOL, выдаёт ≥3 фолда с IS и OOS метриками;
- OOS-метрика для каждого фолда посчитана на параметрах, выбранных на ПРЕДЫДУЩЕМ
  train-окне (нет look-ahead: test-окно строго позже train-окна);
- static-baseline считается; tuned-OOS и static-OOS оба в отчёте;
- тест на МАЛЕНЬКОЙ сетке/коротких окнах ради скорости; детерминизм.

**Verdict (Opus, не Sonnet):** реальна ли деградация IS→OOS; бьёт ли тюнинг
статику OOS; если нет — зафиксировать, что прод должен жить на статических
(или ensemble) параметрах, не на исторически-оптимальных.
