"""
rebalance_v06.py — Scale-in/scale-out + rotation by funding strength, v0.6.

Changes vs v0.5:
  (a) Separate "open new slot" vs "continue ramp" logic.
      - First tranche (open new slot): NO threshold. Pick highest ma12 not already held.
      - Continue ramp (add next tranche): trailing anchor check.
        Each slot tracks entry_ma_anchor (set at first tranche, updated on each
        successful continue). A tranche is added only if current_ma12 >= anchor - tolerance.
        On success, anchor is updated to current_ma12 (trailing, never moves down).
        On failure, fix-A unwind check runs (same as v0.5).
  (b) Removed entry_ma_apr and entry_min_24h_apr from API entirely.
  (c) Added new counters: n_continues (successful continue-ramp adds),
      n_freezes (failed continue-ramp checks).
  (d) New param: continue_tolerance_apr (slack on trailing anchor check).
  (e) best_candidate: no entry-level filters, only "not held" + (optional) must_beat_apr check.

Everything else unchanged from v0.5:
  - 4 slots, n_main_cap=2.
  - State machine empty/growing/holding/shrinking.
  - Holding state: rotation first, else exit if signal < exit_signal_threshold_apr.
  - Frozen-growing unwind (Fix A) — logic identical to v0.5.
  - Aave overlay off by default (aave_apr=0).
  - Per-hour arrays for window metrics.
  - Same data loading (engine.load_data), same constants, same U11.
  - Same aave_only synthetic baseline row.
  - Same metric formulas.

Sweep: 5 configs × 2 periods = 10 rows.

Run: uv run python research/rebalance_v06.py
"""
...
def simulate_rebalance_v06(
    ...
    *,
    ...
    continue_tolerance_apr: float = 0.0,     # slack on trailing anchor check
    rotation_delta_apr: float = 0.10,
    exit_signal_threshold_apr: float = 0.0,
    signal_window_hours: int = 12,
    n_slots_total: int = 4,
    n_main_cap: int = 2,
    aave_apr: float = 0.0,                   # 0 = no Aave overlay (default for v0.6)
    unwind_frozen: bool = True,
) -> tuple:
    """
    Core per-hour simulation loop for v0.6.

    Key differences from v0.5:
    - First tranche: unconditional (picks highest ma12 not held).
    - Continue tranche: trailing anchor check (current_ma12 >= anchor - tolerance).
      Anchor updated to current_ma12 on each successful add.
    - entry_ma_apr and entry_min_24h_apr removed entirely.
    - New counters: n_continues, n_freezes.
    ...
    def make_slot():
        return {
            ...
            "entry_ma_anchor":   0.0,       # NEW: trailing anchor for continue-ramp
        }
    ...
    n_rotations      = 0
    n_ramps          = 0
    n_continues      = 0   # NEW: successful continue-ramp adds
    n_freezes        = 0   # NEW: failed continue-ramp checks
    n_frozen_unwinds = 0
    ...
    def best_candidate(exclude_coins, h, must_beat_apr=None):
        """
        If must_beat_apr is None: pick highest ma12 not in exclude_coins (first-tranche entry).
        If must_beat_apr is set: pick highest ma12 not in exclude_coins that ALSO beats
            must_beat_apr + rotation_delta_apr.

        No entry threshold filter, no 24h-min filter. Just "not held" + (optional) must_beat check.
        """
        best_coin = None
        best_apr  = -1e9
        for c in coins:
    ...
    for h in range(n_hours):
        ...
        if h >= tick_start and (h % tick_hours == 0):
            ...
            for sl in slots:
                ...
                # v0.6 continue-ramp check: trailing anchor
                ...
                if passes:
                    ...
                    # Continue check FAILED — Fix A unwind check (same as v0.5)
                    n_freezes += 1
            ...
            for sl in slots:
                ...
                if n_gh < n_main_cap:
                    held_coins = coins_in_slots()
                    candidate  = best_candidate(held_coins, h)   # no must_beat_apr = first tranche
                    if candidate is not None:
...
def main():
    ...
    # 5 configs (including aave_only synthetic)
    # Each: (name, continue_tolerance_apr, aave_apr, exit_signal_threshold_apr)
    #   None => synthetic aave_only row
    configs = [
        ("aave_only",            None,  None,  None),   # synthetic
        ("v06_strict",           0.00,  0.00,  0.00),   # strict trailing anchor, no slack
        ("v06_slack_2pct",       0.02,  0.00,  0.00),   # 2% APR slack on trailing anchor
        ("v06_slack_5pct",       0.05,  0.00,  0.00),   # 5% APR slack
        ("v06_strict_with_aave", 0.00,  0.05,  0.00),   # strict + Aave overlay
    ]
    ...
    for cfg_name, tolerance_cfg, aave_apr_cfg, exit_thr_cfg in configs:
        ...
        print(f"\nRunning config '{cfg_name}':"
              f" continue_tolerance_apr={tolerance_cfg:.2f} aave_apr={aave_apr_cfg:.2f}"
              f" exit_signal_threshold_apr={exit_thr_cfg:.2f}")

        (pnl, cap, gross_fund_arr, fees_arr, aave_arr, is_idle_arr, info) = simulate_rebalance_v06(
            ...
            n_hours=n_hours,
            continue_tolerance_apr=tolerance_cfg,
            aave_apr=aave_apr_cfg,
            exit_signal_threshold_apr=exit_thr_cfg,
            **default_kwargs,
        )
        ...
        print(f"  peak_cap={peak_cap:.0f}, aave_budget={aave_bud:.0f}, "
              ...
              f"rotations={n_rots}, frozen_unwinds={n_frozen_unwinds}, "
    ...
    col_order = [
        ...
        "n_rotations", "n_frozen_unwinds", "pct_time_in_usdc",
        "tick_hours", "slice_pct",
        "aave_apr", "continue_tolerance_apr",
        "rotation_delta_apr", "exit_signal_threshold_apr",
        "unwind_enabled", "peak_capital",
    ]
    out_path = Path(__file__).parent / "rebalance_v06_results.csv"
    ...
    print("\n" + "=" * 220)
    print("REBALANCE V0.6 — FULL PERIOD (sorted by calmar)")
    print("=" * 220)
    print(df_full[display_cols].sort_values("calmar", ascending=False).to_string(index=False))

    print("\n" + "=" * 220)
    print("REBALANCE V0.6 — LAST 90 DAYS (sorted by calmar)")
    print("=" * 220)
    print(df_90[display_cols].sort_values("calmar", ascending=False).to_string(index=False))

    # ── SANITY CHECK 1: v06_strict_with_aave ≈ v06_strict + aave_income ───────
    print("\n" + "=" * 120)
    print("SANITY CHECK 1: v06_strict_with_aave ≈ v06_strict + aave_income_annual_pct")
    print("Expected: diff < 0.2%")
    print("=" * 120)
    for period in ("full", "last_90d"):
        r_strict     = df_out[(df_out["config"] == "v06_strict")          & (df_out["period"] == period)]
        r_strict_aave = df_out[(df_out["config"] == "v06_strict_with_aave") & (df_out["period"] == period)]
        if r_strict.empty or r_strict_aave.empty:
            continue
        rv_s  = r_strict.iloc[0]
        rv_sa = r_strict_aave.iloc[0]
        expected = rv_s["annual_pct"] + rv_sa["aave_income_annual_pct"]
        actual   = rv_sa["annual_pct"]
        diff = actual - expected
        match = "OK" if abs(diff) <= 0.2 else "MISMATCH"
        print(f"  [{period:8s}]  v06_strict={rv_s['annual_pct']:.2f}%  "
              f"aave_income={rv_sa['aave_income_annual_pct']:.2f}%  "
              f"expected={expected:.2f}%  actual={actual:.2f}%  diff={diff:+.3f}%  [{match}]")

    # ── SANITY CHECK 2: n_continues strict < n_continues slack_5pct ───────────
    print("\n" + "=" * 120)
    print("SANITY CHECK 2: v06_strict n_continues < v06_slack_5pct n_continues")
    print("=" * 120)
    for cfg in ("v06_strict", "v06_slack_2pct", "v06_slack_5pct"):
        r = df_out[(df_out["config"] == cfg) & (df_out["period"] == "full")]
        if r.empty:
            continue
        rv = r.iloc[0]
        print(f"  {cfg:25s}  ramps={rv['n_ramps']:4.0f}  continues={rv['n_continues']:4.0f}"
              f"  freezes={rv['n_freezes']:4.0f}  rotations={rv['n_rotations']:4.0f}"
              f"  annual={rv['annual_pct']:.2f}%  calmar={rv['calmar']:.2f}"
              f"  %usdc={rv['pct_time_in_usdc']:.4f}")

    # ── SANITY CHECK 3: v06_strict vs v05_no_floor_no_aave (closest equiv) ────
    V05_ANNUAL_FULL   = 8.26
    V05_CALMAR_FULL   = 4.03
    V05_ANNUAL_90D    = -1.06
    V05_CALMAR_90D    = -2.33
    print("\n" + "=" * 120)
    print("SANITY CHECK 3: v06_strict vs v05_no_floor_no_aave (closest equivalent)")
    print(f"  v05 reference: full={V05_ANNUAL_FULL:.2f}% calmar={V05_CALMAR_FULL:.2f}  "
          f"90d={V05_ANNUAL_90D:.2f}% calmar={V05_CALMAR_90D:.2f}")
    print("=" * 120)
    for period in ("full", "last_90d"):
        r = df_out[(df_out["config"] == "v06_strict") & (df_out["period"] == period)]
        if r.empty:
            continue
        rv = r.iloc[0]
        v05_ann = V05_ANNUAL_FULL if period == "full" else V05_ANNUAL_90D
        v05_cal = V05_CALMAR_FULL if period == "full" else V05_CALMAR_90D
        print(f"  [{period:8s}]  v06_strict: annual={rv['annual_pct']:.2f}%  calmar={rv['calmar']:.2f}  "
              f"v05_no_floor: annual={v05_ann:.2f}%  calmar={v05_cal:.2f}  "
              f"annual_diff={rv['annual_pct']-v05_ann:+.2f}%  calmar_diff={rv['calmar']-v05_cal:+.2f}")
